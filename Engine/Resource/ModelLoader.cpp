...
namespace Blue
{
	...
	bool ModelLoader::Load(const std::string& name, std::weak_ptr<MeshData>& outData)
	{
		...
		while (!feof(file))
		{
			...
			else if (strcmp(header, "f") == 0) // f = 인덱스
			{
				int v1, v2, v3; 
				int t1, t2, t3;
				int n1, n2, n3;  // 3개씩 조립해서 삼각형
		...
		{
			indices.emplace_back(ix); // 정점데이터와 인덱스 데이터 있으면 매시 데이터 생성 가능
		}
